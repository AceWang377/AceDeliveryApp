...
export const get1stAndToday = () => {
  ...
  const start = yesterday.format('YYYY-MM-DD')
  const end = yesterday.format('YYYY-MM-DD')
  return [start, end]
}
...
// };
export const pastWeek = () => {
  const start = moment().startOf('week').add(1, 'day').format('YYYY-MM-DD')  // Monday
  const end = moment().endOf('week').add(1, 'day').format('YYYY-MM-DD')      // Sunday
  return [start, end]
}
