<template>
  <div class="navbar">
    <div class="statusBox">
      ...
      <span v-if="status===1"
            class="businessBtn">Open Now</span>
      <span v-else
            class="businessBtn closing">Close Now</span>
    </div>
    ...
    <!-- 营业状态弹层 -->
    <el-dialog title="set Business Status"
      ...
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          Open Now
          <span>The restaurant is currently open and will automatically accept any orders. You can click Close to enter the store closing status.</span>
        </el-radio>
        <el-radio :label="0">
          Close Now
          <span>The restaurant is currently closed and only accepts reservations during business hours. You can click Open to manually restore business status.</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer">
  ...
  webSocket() {
    ...
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onmessage = function (msg) {
        ...
        that.$notify({
          title: jsonMsg.type === 1 ? 'Pending Order' : 'Rush the Order',
          duration: 0,
          ...
          message: `${
            jsonMsg.type === 1
              ? `<span>You have a <span style=color:#419EFF>order</span> need to process,${jsonMsg.content},Please accept earlier</span>`
              : `${jsonMsg.content}<span style='color:#419EFF;cursor: pointer'>Process</span>`
          }`,
        })
      }
      // 监听socket错误
