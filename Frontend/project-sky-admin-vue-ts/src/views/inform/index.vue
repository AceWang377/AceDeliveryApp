      ...
      <div class="informList" v-if="baseData.length > 0">
        <div v-for="(item, index) in baseData" :key="index">
          ...
          <div class="item" v-if="item.type === 1">
            <div class="tit">
              <span>【Waiting for orders】</span>{{ item.arrNew[0]
          ...
          <div class="item" v-if="item.type === 2">
            <div class="tit">
              <i>急</i><span>【Waiting for orders】</span>{{ item.arrNew[0]
              }}<span class="fontOrderTip" @click="handleSetStatus(item.id)"
                ><router-link :to="'/order?status=' + 2">{{
                  item.arrNew[1]
                }}</router-link></span
          ...
          >
            <div :class="isActive ? 'titAlready' : ''">
              <div class="tit">
                <span>【Rush Order】</span>{{ item.arrNew[0] }}
                <!-- <span
                  class="fontOrderTip"
                  >去处理</span
                > -->
          ...
          >
            <div :class="isActive ? 'titAlready' : ''">
              <div class="tit">
                <span>【Today's data】</span>Work hard and live well at the same time.<span
                  class="time"
                  >{{ item.createTime }}</span
                >
              </div>
              <div v-if="shopShow && showIndex === index" class="orderInfo">
                ...
                <p>
                  <span
                    ><label>New user today：</label
                    >{{ item.details.newUsers }}</span
                  >
                  <span
                    ><label>Cancel order today：</label
                    >{{ item.details.cancelledOrders }}笔</span
                  >
                  <span
                    ><label>Cancel amount today：</label>￥{{
