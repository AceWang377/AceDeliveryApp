...
export default class extends Vue {
  ...
  private checkphone(rule: any, value: any, callback: any) {
    ...
    if (value == '') {
      ...
      callback(new Error('Please enter right phone number!'))
    } else {
      callback()
    }
  }
  ...
  private validID(rule: any, value: any, callback: any) {
    ...
    if (value == '') {
      ...
      callback(new Error('ID number invalid'))
    }
  }
      ...
      username: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            if (!value) {
              callback(new Error('please enter employee account'))
            } else {
              const reg = /^([a-z]|[0-9]){3,20}$/
              if (!reg.test(value)) {
                callback(new Error('The account number you entered does not match. Please enter 3-20 characters.'))
              } else {
                callback()
              }
            }
  ...
  created() {
    ...
    if (this.$route.query.id) {
      this.title = 'Update employee infomation'
      this.init()
    }
  }
      ...
      if (valid) {
        if (this.actionType === 'add') {
          ...
          editEmployee(params)
            .then((res: any) => {
              if (res.data.code === 1) {
                this.$message.success('Employee updated successful!')
                this.$router.push({ path: '/employee' })
              } else {
                this.$message.error(res.data.msg)
              }
