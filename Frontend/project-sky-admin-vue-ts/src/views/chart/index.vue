  ...
  <div class="dashboard-container">
    ...
    <div class="topLable">
      ...
      <div v-if="act === 'day'" class="dataSelect">
        <div>
          <span @click="checkaffterDate('day', 'before')">Day Before</span>
          <el-date-picker
            v-model="dataTime"
            size="mini"
            type="date"
          ...
          />
          <span @click="checkaffterDate('day', 'after')">Day After</span>
        </div>
        <div><span class="but" @click="init()">View today's data</span></div>
      </div>
      <div v-if="act === 'week'" class="dataSelect">
        <div>
          <span @click="checkaffterDate('week', 'before')">Week Before</span>
          <el-date-picker
            v-model="dataTime"
            size="mini"
            type="week"
          ...
          />
          <span @click="checkaffterDate('week', 'after')">Week After</span>
        </div>
        <div>
          <span class="but" @click="dateAct('week')">View week's report</span>
        </div>
      </div>
      <div v-if="act === 'mouth'" class="dataSelect">
        <div>
          <span @click="checkaffterDate('mouth', 'before')">Month Before</span>
          <el-date-picker
            v-model="dataTime"
            size="mini"
            type="month"
          ...
          />
          <span @click="checkaffterDate('mouth', 'after')">Month after</span>
        </div>
        <div>
          <span class="but" @click="dateAct('mouth')">View month's Data</span>
        </div>
      </div>
    </div>
    <div :key="restKey" class="container">
      <div v-if="act === 'day'" class="topDataBox">
        <div class="box nowData">
          ...
          <div class="item">
            ...
            <div>Compared with the day before 0%</div>
          </div>
        </div>
      ...
      <div class="employeeChart" style="position: relative">
        ...
        <Basic
          ...
          title="Sales Trend by Time Period"
        />
      </div>
    </div>
    <div class="container">
    ...
    <div class="container">
      <div class="chartBox">
        <div>
          <BarChart id="bar" :chart-data="chartDataD" title="In-store Payment Breakdown" />
        </div>
        <div class="itemList">
          <div class="title">Discount Metrics</div>
          <div class="item topLab">
            <span>Discount Total</span><span>{{ discountTotal / 100 }}Pounds</span
            ><span>{{ discountPercentTotal * 100 }}%</span>
          </div>
          <div v-for="(item, index) in discount" :key="index" class="item">
            <span>{{ item.name }}</span
            ><span>{{ item.value / 100 }}Pounds</span
...
export default class extends Vue {
  // fix the dataTime formate
  private dataTime: any = moment().format('YYYY-MM-DD')
  private restKey = 0
